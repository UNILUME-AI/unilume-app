...
**Goal:** Add Clerk-based Google OAuth login to the UNILUME chat app with conversation cloud sync to Neon Postgres.

**Architecture:** Clerk provides authentication via `<ClerkProvider>` in the root layout. The chat page uses `useUser()` to gate message sending — unauthenticated users see a login overlay when they try to send. API routes use `auth()` from `@clerk/nextjs/server` to verify requests. A `proxy.ts` file runs `clerkMiddleware()` so `auth()` works in server components. Conversations are persisted to Neon Postgres keyed by `userId`.

**Tech Stack:** `@clerk/nextjs`, Neon Postgres (`@neondatabase/serverless`), Next.js 16 App Router
...
|------|--------|----------------|
| `src/proxy.ts` | Create | Clerk middleware — makes `auth()` available in server components/routes |
| `src/app/layout.tsx` | Modify | Wrap app with `<ClerkProvider>` |
| `src/app/page.tsx` | Modify | Add login overlay, `<UserButton />`, auth-gated sending |
| `src/app/api/chat/route.ts` | Modify | Add `auth()` check, extract `userId` |
| `src/app/api/feedback/route.ts` | Modify | Add `auth()` check, store `userId` with feedback |
| `src/app/api/conversations/route.ts` | Create | GET/POST endpoints for conversation sync |
| `src/lib/db.ts` | Modify | Add conversation query helpers |
| `scripts/migrate-conversations.sql` | Create | SQL migration for conversations table |

---

### Task 1: Install Clerk and Create Proxy
...
Create `src/proxy.ts` (same level as `app/`, inside `src/`):
...
This is intentionally minimal — we do NOT protect routes in the proxy because our design allows unauthenticated users to view the page. API routes protect themselves individually.

- [ ] **Step 3: Commit**

```bash
git add package.json package-lock.json src/proxy.ts
git commit -m "feat: install @clerk/nextjs and add proxy.ts"
```
...
Key change: `<ClerkProvider>` wraps `<html>`. This is the Clerk-recommended pattern for Next.js App Router.

- [ ] **Step 2: Verify dev server starts**

Run: `npm run dev`
Expected: App starts without errors. No visible UI change yet.
...
### Task 3: Add Auth Check to Chat API Route
...
Add the Clerk import and auth check at the top of the POST handler. Replace the current `POST` function in `src/app/api/chat/route.ts`:
...
git commit -m "feat: protect chat API route with Clerk auth"
```

---

### Task 4: Add Auth Check to Feedback API Route

**Files:**
- Modify: `src/app/api/feedback/route.ts`

- [ ] **Step 1: Add auth() to feedback route**

Replace the full contents of `src/app/api/feedback/route.ts`:
...
Note: This adds a `user_id` column to the feedback INSERT. You may need to add this column to the existing feedback table:
...
git commit -m "feat: protect feedback API route with Clerk auth"
```

---

### Task 5: Create Conversations Database Table and Query Helpers

**Files:**
- Create: `scripts/migrate-conversations.sql`
...
Run the migration using your DATABASE_URL. This can be done via the Neon dashboard SQL editor, or:

```bash
psql "$DATABASE_URL" -f scripts/migrate-conversations.sql
```

- [ ] **Step 3: Add conversation helpers to db.ts**
...
```bash
git add scripts/migrate-conversations.sql src/lib/db.ts
git commit -m "feat: add conversations table and query helpers"
```

---

### Task 6: Create Conversations API Route
...
git commit -m "feat: add conversations API route for cloud sync"
```

---

### Task 7: Update Chat Page — Auth-Gated Sending, Login Overlay, UserButton

**Files:**
- Modify: `src/app/page.tsx`

This is the largest task. It modifies the existing chat page to:
1. Add `useUser()` for auth state
2. Add `<UserButton />` in the header
3. Add login overlay with `<SignIn />`
4. Gate `send()` behind auth check
5. Replace localStorage sync with cloud sync
6. Generate a conversation ID for cloud persistence
...
Run: `npm run dev`
Expected: Page renders. Header shows "新对话" but no UserButton (not logged in yet). Clicking send or a quick action should show the Clerk SignIn overlay.

- [ ] **Step 3: Commit**

```bash
git add src/app/page.tsx
git commit -m "feat: add auth-gated sending, login overlay, UserButton, cloud sync"
```

---

### Task 8: Configure Clerk Environment Variables

This task requires manual steps in the Clerk Dashboard and Vercel Dashboard.

- [ ] **Step 1: Set up Clerk project**

1. Go to [clerk.com](https://clerk.com) and create a new application (or use existing)
2. In the Clerk Dashboard → **Configure → Social Connections** → enable **Google**
3. Disable all other sign-in methods (email, phone) so only Google appears
4. Copy the **Publishable Key** and **Secret Key** from **API Keys**

- [ ] **Step 2: Add environment variables locally**
...
2. See the chat UI with welcome message
3. Click a quick action → login overlay appears with Google button
4. Complete Google sign-in → overlay closes, message sends
5. Header shows avatar + dropdown menu
6. Click "新对话" → starts a new conversation
7. Refresh page → conversation loads from database

- [ ] **Step 5: Commit .env.local to .gitignore (verify)**

Confirm `.env.local` is in `.gitignore`. It should already be there for a Next.js project, but verify:
...
Execute `scripts/migrate-conversations.sql` against your Neon database. Use the Neon Dashboard SQL editor or:
...
Expected: Shows the conversations table with columns `id`, `user_id`, `messages`, `created_at`, `updated_at`.
